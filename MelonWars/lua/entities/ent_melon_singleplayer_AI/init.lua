...
function ENT:CheckDefeat()
	...
	for k, v in pairs (allents) do
		if (v.Base == "ent_melon_base") then
			if (v:GetNWInt("melonTeam", -1) == 0) then
				return false
			end
		end
	end
...
function ENT:SpawnUnit(unit_class, path)
	...
	melonTeam = self:GetNWInt("melonTeam", 0)
	newMarine.population = 0
	newMarine:Spawn()
	newMarine:SetNWInt("melonTeam", self:GetNWInt("melonTeam", 0))
...
function ENT:ReadWord(word)
	...
	elseif (exp[1] == "addcredits") then
		local credits = teamCredits[util.StringToType(exp[2], "int")]+util.StringToType(exp[3], "int")
