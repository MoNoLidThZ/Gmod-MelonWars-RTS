...
function MW_CalculateConnections(ent, all)
	timer.Simple(0.05, function ()
		...
		--PrintTable(mw_electric_network)
	end)
end
				...
				if (istable(searched)) then
					if (found == false) then
						if (v.alreadySet == false) then
							...
							print("Taking "..tostring(separatedEnergy).." from network "..tostring(ambassador.network).." to "..tostring(newNetwork))
...
function ENT:GivePower(power)
	...
	if (self.network > 0) then
		...
		if (nw.energy >= nw.capacity) then
			can = false
			nw.energy = nw.capacity
		end
	end
	return can
end